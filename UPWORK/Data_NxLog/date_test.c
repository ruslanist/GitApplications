...
int main(int argc UNUSED, const char * const *argv, const char * const *env UNUSED)
{
    ...
    nx_date_set_parsedateutc(TRUE);
    nx_date_set_genereatedateutc(TRUE);
    ASSERT(nx_date_parse_iso(&timeval, "1970-05-07 16:39:13,42", NULL) == APR_SUCCESS);
    compare_timeval(timeval, "1970-05-07 16:39:13");
    ASSERT(timeval == 10946353420000LL);

    nx_date_set_parsedateutc(FALSE);
    nx_date_set_genereatedateutc(FALSE);
    ...
    ASSERT(nx_date_parse_iso(&timeval, "2011-12-06T19:14:15.000003000-01:00", NULL) == APR_SUCCESS);
    ASSERT(timeval == 1323202455000003LL);
    ...
    // test aruba
    const char *end = NULL;
    ASSERT(nx_date_parse_aruba(&timeval, "Nov 3 14:50:30 2015 xxx", &end) == APR_SUCCESS);
    ASSERT(end != NULL);
    ASSERT(strcmp(end, " xxx") == 0);
    ASSERT(nx_date_to_iso(tmpbuf, sizeof(tmpbuf), timeval) == APR_SUCCESS);
    ASSERT(strcmp("2015-11-03 14:50:30", tmpbuf) == 0);
    ...
    ASSERT(nx_date_parse_aruba(&timeval, "Nov 30 23:59:59 2015", &end) == APR_SUCCESS);
    ASSERT(end != NULL);
    ASSERT(strcmp(end, "") == 0);
    ASSERT(nx_date_to_iso(tmpbuf, sizeof(tmpbuf), timeval) == APR_SUCCESS);
    ASSERT(strcmp("2015-11-30 23:59:59", tmpbuf) == 0);

    ASSERT(nx_date_parse(&timeval, "Nov 30 23:59:59 2015", &end) == APR_SUCCESS);
    ASSERT(end != NULL);
    ASSERT(strcmp(end, "") == 0);
    ASSERT(nx_date_to_iso(tmpbuf, sizeof(tmpbuf), timeval) == APR_SUCCESS);
    ASSERT(strcmp("2015-11-30 23:59:59", tmpbuf) == 0);
    ...
    ASSERT(timeval != 0);
    ASSERT(nx_date_to_rfc3164(tmpbuf, sizeof(tmpbuf), FALSE, timeval) == APR_SUCCESS);
    ...
    timeval = 1061691255000003LL;
    ASSERT(nx_date_to_string(tmpbuf, sizeof(tmpbuf), NULL, &retsize, timeval) == APR_SUCCESS);
    ASSERT(nx_date_to_string(tmpbuf, sizeof(tmpbuf), NULL, NULL, timeval) == APR_SUCCESS);
    ASSERT(nx_date_to_string(tmpbuf, sizeof(tmpbuf), "YYYY-MM-DD hh:mm:ss", &retsize, timeval) == APR_SUCCESS);
    ASSERT(nx_date_to_string(tmpbuf, sizeof(tmpbuf), "YYYY-MM-DD hh:mm:ss", NULL, timeval) == APR_SUCCESS);

    // test binddate
    ASSERT(nx_date_parse_binddate(&timeval, "23-Mar-2017 06:38:30.143", NULL) == APR_SUCCESS);
    ASSERT(nx_date_parse_binddate(&timeval2, "23-Mar-2017  06:38:30", NULL) == APR_SUCCESS);
    ASSERT(nx_date_to_iso(tmpbuf, sizeof(tmpbuf), timeval) == APR_SUCCESS);
    ASSERT(strcmp("2017-03-23 06:38:30", tmpbuf) == 0);
    ASSERT(timeval - timeval2 == 143000);

    ASSERT(nx_date_parse_binddate(&timeval, "2017-Mar-23   06:38:30.143", NULL) == APR_SUCCESS);
    ASSERT(nx_date_parse_binddate(&timeval2, "2017-Mar-23 06:38:30", NULL) == APR_SUCCESS);
    ASSERT(nx_date_to_iso(tmpbuf, sizeof(tmpbuf), timeval) == APR_SUCCESS);
    ASSERT(strcmp("2017-03-23 06:38:30", tmpbuf) == 0);
    ASSERT(timeval - timeval2 == 143000);

    ASSERT(nx_date_parse_binddate(&timeval, "23-Mar-2017 23:38:30.143", NULL) == APR_SUCCESS);
    ASSERT(nx_date_to_iso(tmpbuf, sizeof(tmpbuf), timeval) == APR_SUCCESS);
    ASSERT(strcmp("2017-03-23 23:38:30", tmpbuf) == 0);


    // test dnslog
    ASSERT(nx_date_parse_win_dnslog(&timeval, "3/13/2017 8:46:01 AM", NULL) == APR_SUCCESS);
    ASSERT(nx_date_to_iso(tmpbuf, sizeof(tmpbuf), timeval) == APR_SUCCESS);
    ASSERT(strcmp("2017-03-13 08:46:01", tmpbuf) == 0);
